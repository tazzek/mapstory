...
export default function EditorPage() {
    ...
    return (
        <div className="flex h-screen bg-white overflow-hidden font-sans mobile:flex-col-reverse">

            {/* 1. NAVIGATION RAIL */}
            <div
                className={`transition-all duration-500 ease-in-out border-r border-white bg-[#EBE9E4] overflow-y-auto flex-shrink-0 ${isFocusMode ? 'w-0 opacity-0 overflow-hidden mobile:w-full mobile:h-0 mobile:border-t-0' : 'w-[230px] opacity-100 tablet:w-[100px] mobile:w-full mobile:h-[90px] mobile:border-r-0 mobile:border-t mobile:z-50 mobile:bg-[#EBE9E4] mobile:pb-safe mobile:overflow-visible'
                    }`}
            >
                <NavigationRail />
            </div>
            ...
            {/* 2. SECONDARY PANEL */}
            <div
                className={`relative transition-all duration-500 ease-in-out border-r border-vintage-border/30 bg-gradient-to-b from-white to-vintage-warm/10 flex-shrink-0 ${isFocusMode ? 'w-0 opacity-0 overflow-hidden mobile:translate-y-full mobile:h-0 mobile:w-full' : 'w-[350px] opacity-100 tablet:w-[320px] mobile:absolute mobile:bottom-[90px] mobile:left-0 mobile:w-full mobile:h-[35vh] mobile:z-40 mobile:rounded-t-3xl mobile:shadow-[0_-5px_25px_rgba(0,0,0,0.1)] mobile:bg-white'
                    }`}
            >
                ...
                <div
                    className="px-8 pb-8 mobile:pt-8 mobile:px-5 mobile:overflow-y-auto mobile:h-full"
            ...
            {/* 3. MAIN CANVAS */}
            <div className={`flex-1 relative bg-gradient-to-br from-stone-50 to-stone-100 overflow-hidden transition-all duration-500 mobile:w-full ${isFocusMode ? 'mobile:h-screen' : 'mobile:pb-[35vh]'}`}>
                <MapCanvas />

                {/* Sidebar Collapse Toggle (Focus Mode) - widoczny tylko gdy zwinine */}
                {isFocusMode && (
                    <div className="absolute top-1/2 -translate-y-1/2 left-4 z-30 mobile:top-4 mobile:left-auto mobile:right-4 mobile:hidden">
                        <button
                            onClick={toggleFocusMode}
                ...
                {isFocusMode && (
                    <div className="hidden mobile:flex absolute bottom-6 left-1/2 -translate-x-1/2 z-30 animate-fade-in-up">
                        <button
                            onClick={toggleFocusMode}
                ...
                <div className="absolute top-4 right-4 z-30 mobile:top-4 mobile:right-4">
                    <ShareAction />
                </div>

                {/* Bottom left: Zoom */}
                <div className="absolute bottom-6 left-6 z-30 transition-all duration-500 tablet:bottom-6 mobile:hidden">
                    <ZoomUiWidget />
                </div>

                {/* Bottom right: Price + Format */}
                <div className="absolute bottom-6 right-6 z-30 flex flex-col items-end gap-3 transition-all duration-500 tablet:bottom-6 mobile:top-4 mobile:left-4 mobile:right-auto mobile:bottom-auto mobile:items-start mobile:flex-col-reverse">
                    <PriceWidget />
                    <FormatWidget />
                </div>
            </div>
